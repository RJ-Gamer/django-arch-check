...
def test_high_stakes_task_is_critical(
    proj: ProjectBuilder, name: str, keyword: str
) -> None:
    ...
    ), f"{name!r} should be critical (keyword={keyword!r})"
...
@pytest.mark.parametrize(
    "kwarg", ["max_retries=3", "autoretry_for=(Exception,)", "retry_backoff=True"]
)
